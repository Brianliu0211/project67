...
- **`schedule_events`** (日曆行程)：`id` (PK), `profile_id` (FK), `start_at`, `end_at`, `title`, `location`, `latitude`, `longitude`, `google_event_id`, `google_calendar_id`, `sync_status` (`local_only`, `synced`, `pending_push`), `last_synced_at` (具備經緯度檢查與 Google 日曆專屬次日曆「保險助手」雙向同步)。
  - **🛡️ 類型白名單與手機手感 (2026-09-22)**：前端全面對齊 `chk_schedule_events_event_type` 白名單 (`customer_visit`, `personal`, `meeting`, `follow_up`, `general`) 消除 23514 報錯；手機端行程表單與多站路線規劃全面升級為「底部抽屜 (Bottom Sheet)」，配置大拇指操作熱區、`viewInsets.bottom` 防遮擋推移、頂欄刷新按鈕與手勢下拉更新 (Pull-to-Refresh)。

### B. 雲端存儲空間與 Edge Functions 安全 (Supabase Storage & Functions)
- **`avatars`** (業務員頭像 Bucket)
- **`customer-photos`** (客戶頭像 Bucket)
- **`crawl-insurance-products`**：雙軌驗證 (排程限定 `$CRAWLER_CRON_SECRET` 伺服器環境變數；手動限定 Admin/Dev JWT 授權)，包含全套跨網域 CORS Header 與 OPTIONS 預檢，強制使用 Service Role Key 寫入 `policy_clauses`，未授權立即 401 攔截。
- **`transcribe-voice` & `voice-scheduler`**：啟用 `verify_jwt: true` 且後端嚴格檢查非空 Bearer Token 與身分驗證，未授權立即 401 攔截。
...
- **🛡️ 跨使用者快取隔離 (2026-08-31)**：`OfflineDataStore` 掛載 `ensureOwner(userId)` 與 `clear()`，當使用者切換或登出時立即清空記憶體快取，防止不同帳號間客戶/行程資料洩漏。

---

## 1. 🔑 驗證與防護門模組 (Auth & Status Gate)
- **按鈕：[Email 註冊]** -> 移除姓名自動預填，密碼具備眼睛防窺切換，註冊成功彈窗提供「前往收件匣 (開啟信箱)」快速跳轉。
- **常用信箱網域快選** -> 支援 Gmail, Yahoo, Outlook, iCloud 網域一鍵帶入，封裝防溢出保護。
- **按鈕：[Google 登入]** -> **(⚠️ 隱患：跳過填寫 team_code，會產生幽靈帳號)**。
- **🛡️ 資安加固與商業純淨版 (2026-08-31)** -> 前端徹底移除明文 Demo 帳密與秘密繞過後門，商業發行版回歸正規驗證；離線模式支援平滑降級預覽。
...
- **【Deleted (已軟刪除)】狀態**：使用者提出刪除後進入待清理狀態，等待開發者於控制台進行 Hard Delete 實體清除；個人頁彈窗指引聯絡管理員由後端受控流程處理。
...
### A. 業務戰情與分析 (`data_dashboard_tab.dart` / `policy_comparison_screen.dart`)
- **圖表/指標**：名下客戶總量（🌟保戶 / 💬跟進中 / 🎯未開發 3 大生命週期漏斗）、本月拜訪達成率、推動中專案數（100% 依據 Supabase 實體資料庫動態計算）。
- **商品知識中樞**：12,090 筆全台官方條款庫（SQL 即時計數零假資料）、15 大險種智慧擴展檢索矩陣、伺服器端 Range 高效分頁。
- **操作：[商品橫向並排比較]** -> 底部亮橘色浮動列（`已選 X/8 款商品`）$\rightarrow$ 2~8 款商品橫向並排規格表（支援「只看差異」開關與原廠 PDF 條款直開）。
- **表單/彈窗：[全真醫療單據試算]** -> 6 大臨床醫療情境範本快速載入 + 自由單據明細編輯（病房天數/單價、手術自費、標靶藥品/醫材雜費）+ 健保 2-2-7 處置陷阱即時檢核。
- **表單/彈窗：[🔥 跨保單組合包聯合精算]** -> 支援多張保單同時出險之多層次理賠流向瀑布（正本核銷 $\rightarrow$ 副本補足 $\rightarrow$ 正本收據衝突預警 $\rightarrow$ 最終自費缺口）。
- **操作：[💼 為名下客戶規劃]** -> 比較表內一鍵彈窗選取名下客戶，直接寫入 `customer_policies` 實體保單表，完成 CRM 業務閉環。
...
- **[多選操作列 Batch Action Bar]** -> 支援 RWD 雙軌佈局：電腦版（>= 768px）單行橫排；手機版（< 768px）切換為方案 A 雙層等寬均分佈局（上層：全選計數與退出；下層：3 顆均分按鈕「匯出」、「建立專案」、「移至垃圾桶」，內建 `FittedBox` 防溢出保護與 44px 觸控熱區）。
...
### D. 拜訪專案與行程 (`visit_projects_tab.dart` / `home_screen.dart`)
- **[拜訪專案清單]** -> 具備展開/收合客戶核取清單的功能。
- **[今日行程 / 日程表戰情中樞]** -> 支援「月網格」、「日時間軸」與「議程清單」三軌視圖切換與持久化。月網格下右欄為選定日期 24h 時間軸詳情與 7 天連貫列位演算法；日時間軸下右欄為小月曆導航面板與 00:00 起始滿版刻度；議程清單滿版無右欄且自動過濾過期歷史行程；**手機端頂部導航列 (AppBar) 常駐「+」手動新增行程按鈕與「重新整理」按鈕，全視圖支援手勢下拉更新 (Pull-to-Refresh)；行程表單與多站路線規劃於手機端全面升級為「底部抽屜 (Bottom Sheet)」，配置大拇指操作熱區與鍵盤防遮擋自動推移**；整合語音智慧排程、多站順路路線導覽與 Google 日曆雙向同步預備層。
