...
- **`tags`** (標籤庫)：`id` (PK), `category_id` (FK), `name` (全團隊唯讀，管理限 admin/dev)
- **`customer_tags`** (客戶標籤多對多)：`customer_id` (FK), `tag_id` (FK) (業務員僅可管理自身客戶之標籤)
...
- **`crawl-insurance-products`**：雙軌驗證 (`x-crawler-secret: $CRAWLER_CRON_SECRET` + Admin JWT)，強制使用 Service Role Key 寫入，未授權立即 401 攔截。
