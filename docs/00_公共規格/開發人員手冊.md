...
為確保專案程式碼的穩定性，並避免多人同時修改同一檔案造成嚴重的衝突（Merge Conflicts），本專案採用**分支協作模式**。以下為「開工」到「收工」的操作步驟，同時提供 **GitHub Desktop 介面** 與 **Git CLI 指令** 雙重對照。

### 📌 步驟一：開工（同步最新程式碼並開闢特徵分支）

每次準備開始寫程式前，必須先將雲端的最新程式碼同步到本地，並建立獨立的分支。
...
5. 輸入您的分支名稱，命名規範：`feature/您的名字-功能簡稱`（例如 `feature/lobo-customer-ui` 或 `feature/beast-auth`），點擊 **Create Branch**。
...
### 📌 步驟三：收工（驗證、提交、撰寫日誌與發起合併）

開發完成一個階段，準備休息或移交工作時，請**嚴格遵循**以下「收工步驟」：

#### 1. 本地代碼驗證 (自救第一步)
在提交前，請先執行以下指令以確保您的代碼沒有語法錯誤或警告：
```bash
# 執行靜態代碼分析 (警告必須盡可能修復)
flutter analyze
...
#### 2. 提交變更 (Commit & Push)
*提交訊息格式請參照 [Git提交訊息規範.md](file:///c:/Users/USER/Documents/GitHub/project67/docs/00_%E5%85%AC%E5%85%B1%E8%A6%8F%E6%A0%BC/Git%E6%8F%90%E4%BA%A4%E8%A8%8A%E6%81%AF%E8%A6%8F%E7%AF%84.md)*。

* **GitHub Desktop 操作**：
  1. 在左側 Change 清單中勾選要提交的檔案。
  2. 在左下角填寫 **Summary**（主標題，例如 `feat: 新增客戶卡片 UI 元件`）與 **Description**（詳細說明）。
  3. 點擊 **Commit to feature/...**。
  4. 點擊頂部的 **Push origin** 將代碼推送到雲端。
* **Git CLI 指令**：
  ```bash
  git add .
  git commit -m "feat: 您的變更主標題" -m "詳細的變更內容描述說明"
  git push origin feature/您的名字-功能簡稱
  ```

#### 3. 撰寫開發日誌
根據 `.agents/AGENTS.md` 與 [開發日誌規範.md](file:///c:/Users/USER/Documents/GitHub/project67/docs/03_%E9%96%8B%E7%99%BC%E6%97%A5%E8%AA%8C/%E9%96%8B%E7%99%BC%E6%97%A5%E8%AA%8C%E8%A6%8F%E7%AF%84.md)，您或您的 AI 助理必須在 `docs/03_開發日誌/` 資料夾下，新增一份以 `YYYYMMDD_功能簡稱.md` 命名的文件，完整記錄本次的變更與驗證方式。

#### 4. 發起 Pull Request (PR) 進行合併
1. 前往專案的 GitHub 網頁。
2. 系統會顯示黃色橫幅提醒「`feature/... had recent pushes...`」，點擊 **Compare & pull request**。
3. 檢查變更無誤後，填寫 PR 內容並點擊 **Create pull request**。
4. 通知您的夥伴進行審查（Review）與合併（Merge）。
5. **分支清理**：合併成功後，請在本地切換回 `main`，拉取最新代碼並刪除該特徵分支：
   - GitHub Desktop：點擊 Current Branch ➔ 選擇 `main` ➔ 點擊 Pull ➔ 再點擊 Branch 選單 ➔ 選擇 Delete 刪除本地特徵分支。
   - CLI：
     ```bash
     git checkout main
     git pull origin main
