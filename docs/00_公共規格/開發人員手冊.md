...
> [!NOTE]
> **💻 蘿蔔筆電特有雲端工作區與對應角色防呆說明**：
> - **蘿蔔雲端工作區路徑**：`h:\我的雲端硬碟\巨獸來襲\LOBO資料夾\` (此為蘿蔔筆電特有掛載路徑)。
> - **重要稱呼與資料夾防呆**：
>   - 🐲 **「龍兄」**：指 Google 雲端硬碟 (`h:\我的雲端硬碟\巨獸來襲\LOBO資料夾\`) 中的成員檔案/資料夾（例如 `龍兄.gdoc`），**非** Git 專案目錄下的 `02_巨獸_工作區`。
>   - 👾 **「巨獸 (Beast)」**：指 Git 專案協作開發者夥伴（對應分支前綴 `feature/beast-*` 與專案 [docs/02_巨獸_工作區/](../02_巨獸_工作區/)）。
...
### 🔄 階段一：開工前（對 AI 輸入「開工」取得分支引導與防呆，參見 [開發人員快捷指令](開發人員快捷指令.md)）
...
### 🌐 額外指南：啟動本地網頁測試版預覽 (「讓我看看」，參見 [開發人員快捷指令](開發人員快捷指令.md))
...
### 📌 步驟三：收工（驗證、喚醒 AI 產出日誌、提交與 GitHub 網頁合併，參見 [開發人員快捷指令](開發人員快捷指令.md)）
...
- **自動執行 Git 變更檢視與智慧分流**：
  - 🟢 **純文件模式 (Fast-Track)**：若變更 100% 僅限於 `docs/` 或 `.md` 文件，提示可免發起 PR，直接提交並 Push 至 `main`。
  - 🔴 **程式碼模式 (Standard)**：若包含 `lib/` 或 `pubspec.yaml` 等程式碼，強制要求使用特徵分支並引導發起 PR。
- 自動在 `docs/03_開發日誌/` 下根據規範為您建立本次的開發日誌檔案。
- 自動更新並勾選 `docs/進度.md` 中的已完成項目。
- 自動在對話中為您產出可以直接複製貼上的 **Git Commit Summary** 與 **Description**。
- 自動輸出對應的 GitHub 推送與合併引導。
...
### 🔴 鐵律：優先查看並更新「專案緊急救援區」
> [!IMPORTANT]
> 為了避免資訊斷聯與減少重複，我們已將蘿蔔與巨獸工作區的救援區合併為公共規格下的統一狀態看板：
> - 專案緊急救援區：[docs/00_公共規格/緊急救援區.md](緊急救援區.md)
>
> **當任何一人遇到危機（如 Token 用盡、本地代碼炸掉無法編譯）時，請第一時間在「緊急救援區」中更新自己的狀態並留下錯誤日誌，並通知夥伴。夥伴收到通知後，必須「優先前往緊急救援區查看需要接手或救援的事項」，並予以協助！**
...
1. **第一步：判定與隔離 (Triage & Isolation)**
   ...
   - 避免危機蔓延：如果是資料庫異常，立即在 LINE 通知夥伴，並在統一的「[緊急救援區](緊急救援區.md)」中新增說明，防止夥伴推送代碼導致資料覆蓋。
...
3. **第三步：緊急自救/支援 (Rescue & Collaboration)**
   - 遇到無法編譯或嚴重 Error，請立即在統一的「[緊急救援區](緊急救援區.md)」更新您的狀態，貼上完整 Trace log 與出問題的分支。在與 AI 開發對話時，亦可使用 [確認狀況](開發人員快捷指令.md) 啟動安全凍結，避免程式碼被 AI 進一步污染。
   - 夥伴接手時，**優先檢視緊急救援區中的日誌**，依據日誌進行代碼回滾（`git revert`）或分支接手。
...
1. **嚴禁使用絕對路徑 (`file:///`)**：
   ...
   * *原因*：絕對路徑包含個人電腦使用者名稱，一旦換電腦點擊就會失效，且 Obsidian 無法將其識別為 Vault 內部節點，導致關係圖斷聯。
2. **優先使用相對路徑與大小寫對齊**：
   * 🟢 *標準格式*：`[顯示名稱](../相對路徑/檔名.md)`
   * *注意*：路徑大小寫必須與實體硬碟完全一致（一律使用小寫 `docs/`），以防 GitHub 網頁版 404 錯誤。
3. **亦可使用 Wiki-Links 雙括號（但禁止建立概念型空連結）**：
   * 🟢 *Wiki-Links 格式*：`[[進度.md]]` 或 `[[00_公共規格/專案通用指令.md|專案通用指令]]`。
   * ❌ *禁止做法*：嚴禁在句子中隨意將抽象名詞加上 Wiki-Link（如 `[[設計]]`、`[[功能]]`）。只有當該檔案實際存在於 `docs/` 資料夾時，才能使用雙括號建立實體連結，避免 Obsidian 關係圖產生無效空節點。
