...
### 🔄 階段一：開工前（對 AI 輸入「開工」取得分支引導）

1. **對 AI 輸入「開工」**：在準備開始寫程式前，請在聊天視窗中對 AI 助理輸入「**開工**」。
2. **討論目標**：AI 助理會主動詢問您本次要開發的功能與目標（如：「我現在想寫登入頁面的 UI 刻劃」）。
3. **取得引導**：與 AI 轉達共識後，AI 會自動為您計算出本次建議的**分支名稱**（格式如 `feature/username-featurename`）以及對應的 Git 開工指令。
4. **建立分支**：此時，專案人員即可依據引導進行開工。
...
5. 輸入 AI 建議的分支名稱（例如 `feature/lobo-customer-ui` 或 `feature/beast-auth`），點擊 **Create Branch**。
